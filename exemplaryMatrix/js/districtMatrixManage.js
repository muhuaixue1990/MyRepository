...
$(function(){
	...
	//退回补录按钮
	$("#matrixList").on("click",".sendBack",function(){
		var sendBackParam= $(this).attr("name");
		var sendBackParamArray = sendBackParam.split(',')
		...
		layer.open({
			...
			btn1: function(index, layero){
				//按钮确定的回调
				sendBack(sendBackParamArray[0],sendBackParamArray[1])
				layer.close(index);
			},
	...
	$("#exportQuarterStatement").click(function(){
		window.location.href = rootPath+"/exemplaryMatrixBackstage/exportMatrixQuarterStatement.action?district="+district+"&year="+$("#year").val()+"&quarter="+$("#quarter").val()+"&auditType=1";
	})
	
	//导出年度报表按钮
	$("#exportYearStatement").click(function(){
		window.location.href = rootPath+"/exemplaryMatrixBackstage/exportMatrixYearStatement.action?district="+district+"&year="+$("#year").val()+"&quarter=4&auditType=1";
	})
	
	//查看基地年度报表详情
	$("#matrixList").on("click",".exeportData",function(){
		var matrixCode = $(this).attr("name");
		window.location.href = rootPath+"/exemplaryMatrixBackstage/exportMatrixStatement.action?matrixCode="+matrixCode+"&year="+$("#year").val()+"&quarter="+$("#quarter").val()+"&auditType=1";;
	})
 ...
 * @param matrixCode
 */
function sendBack(matrixCode,applicationYear){
	var params = {"params.userCode":matrixCode,
					"params.applicationYear":applicationYear}
	$.ajax({
		type: "post",
		url: rootPath+"/exemplaryMatrixApplication/sendBackMatrixInfo.action",
		async : false,
		timeout : 30000,
		data : params,
		dataType:'json',
...
//					html += '<td  width=""><a class="color_blue backInfo sendBack" name="'+exemplaryMatrixApplicationList[i].userCode+','+exemplaryMatrixApplicationList[i].applicationYear+'">退回补录信息</a></td>'
					html += '</tr>'
				}
			}else{
