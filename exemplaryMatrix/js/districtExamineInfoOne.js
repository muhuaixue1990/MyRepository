...
$(function(){
	...
	if(examineFlag == "-1"){
		...
		$("#nextStep").click(function(){
			if(examineFlag == "1"){
				//推荐
				window.location.href = rootPath + "/page/jsp/exemplaryMatrix/districtExamineInfoTwo_new.jsp?applicationUserCode="+applicationUserCode+"&applicationId="+applicationId;
			}else if(examineFlag == "0"){
				//查看
				window.location.href = rootPath + "/page/jsp/exemplaryMatrix/districtExamineInfoTwoDetail.jsp?applicationUserCode="+applicationUserCode+"&applicationId="+applicationId;
			}
		})
	}
	...
	$.ajax({
		...
		success: function(data, textStatus){
			...
			if(1==status){
				...
				//法人代表
				$("#legalRepresentative").text(enterpriseInfo.legalRepresentative==null?"-":enterpriseInfo.legalRepresentative );
				//注册资本
				$("#registeredCapital").text(enterpriseInfo.registeredCapital==null?"-":enterpriseInfo.registeredCapital);
				//企业网址
				$("#website").text(enterpriseInfo.website==null?"-":enterpriseInfo.website);
			}
		}
	})
}
	...
	$.ajax({
		...
		success: function(data, textStatus){
			...
			if(status == "1"){
				...
				$("#establishTime").text(exemplaryMatrixApplication.establishTime==null?"-":exemplaryMatrixApplication.establishTime);
				$("#matrixAddress").text(exemplaryMatrixApplication.matrixAddress==null?"-":exemplaryMatrixApplication.matrixAddress);
				$("#competentDepartment").text(exemplaryMatrixApplication.competentDepartment==null?"-":exemplaryMatrixApplication.competentDepartment);
				$("#projectArea").text(exemplaryMatrixApplication.projectArea==null?"-":exemplaryMatrixApplication.projectArea);
				$("#buildingArea").text(exemplaryMatrixApplication.buildingArea==null?"-":exemplaryMatrixApplication.buildingArea);
				$("#ownArea").text(exemplaryMatrixApplication.ownArea==null?"-":exemplaryMatrixApplication.ownArea);
				$("#rentArea").text(exemplaryMatrixApplication.rentArea==null?"-":exemplaryMatrixApplication.rentArea);
				$("#commonalityArea").text(exemplaryMatrixApplication.commonalityArea==null?"-":exemplaryMatrixApplication.commonalityArea);
				$("#personnelQuantity").text(exemplaryMatrixApplication.personnelQuantity==null?"-":exemplaryMatrixApplication.personnelQuantity);
				$("#serviceStaff").text(exemplaryMatrixApplication.serviceStaff==null?"-":exemplaryMatrixApplication.serviceStaff);
				$("#instructorQuantity").text(exemplaryMatrixApplication.instructorQuantity==null?"-":exemplaryMatrixApplication.instructorQuantity);
				$("#facilitatingAgency").text(exemplaryMatrixApplication.facilitatingAgency==null?"-":exemplaryMatrixApplication.facilitatingAgency);
				$("#enterpriseQuantity").text(exemplaryMatrixApplication.enterpriseQuantity==null?"-":exemplaryMatrixApplication.enterpriseQuantity);
				
				$("#opticalFiber").text(exemplaryMatrixApplication.opticalFiber==null?"-":exemplaryMatrixApplication.opticalFiber);
				$("#broadband").text(exemplaryMatrixApplication.broadband==null?"-":exemplaryMatrixApplication.broadband);
				$("#wifi").text(exemplaryMatrixApplication.wifi==null?"-":exemplaryMatrixApplication.wifi);
				...
				$("#goalOfDevelopment").html(exemplaryMatrixApplication.goalOfDevelopment==null?"-":goalOfDevelopment);
				...
				$("#feeScale").html(exemplaryMatrixApplication.feeScale==null?"-":feeScale);
				...
				$("#modelSelfreport").text(exemplaryMatrixApplication.modelSelfreport==null?"-":exemplaryMatrixApplication.modelSelfreport);
				
				$("#providePlace").text(exemplaryMatrixApplication.providePlace==null?"-":exemplaryMatrixApplication.providePlace)
		...
		success: function(data, textStatus){
			...
			if(status == "1"){
				...
				for(var i=0;i<businessCircumstances.length;i++){
					if(businessCircumstances[i].year == null &&
							businessCircumstances[i].totalAssets == null &&
							businessCircumstances[i].operationRevenue == null &&
							businessCircumstances[i].enterpriseQuantity == null &&
							businessCircumstances[i].smallMicroEnterpriseQuantity == null &&
							businessCircumstances[i].proportion == null &&
							businessCircumstances[i].personnelQuantity == null){
						continue;
					}
					html += '<tr>';
					html += '<td width="10%">'+ (businessCircumstances[i].year==null?'-':businessCircumstances[i].year) +'</td>';
					html += '<td width="12%">'+(businessCircumstances[i].totalAssets==null?'-':businessCircumstances[i].totalAssets) +'</td>';
					html += '<td width="15%">'+(businessCircumstances[i].operationRevenue==null?'-':businessCircumstances[i].operationRevenue) +'</td>';
					html += '<td width="15%">'+(businessCircumstances[i].enterpriseQuantity==null?'-':businessCircumstances[i].enterpriseQuantity) +'</td>';
					html += '<td width="15%">'+(businessCircumstances[i].smallMicroEnterpriseQuantity==null?'-':businessCircumstances[i].smallMicroEnterpriseQuantity) +'</td>';
					html += '<td width="18%" >'+(businessCircumstances[i].proportion==null?'-':businessCircumstances[i].proportion) +'</td>';
					html += '<td width="15%" >'+(businessCircumstances[i].personnelQuantity==null?'-':businessCircumstances[i].personnelQuantity) +'</td>';
					html += '</tr>';
				}
		...
		success: function(data, textStatus){
			...
			if(status == "1"){
				...
				for(var i=0;i<cooperationFacilitatingAgencies.length;i++){
					if(cooperationFacilitatingAgencies[i].cooperativeFacilitatingAgencyName==null &&
							cooperationFacilitatingAgencies[i].serviceItems == null &&
							cooperationFacilitatingAgencies[i].signingTime == null){
						continue;
					}
					
					html += '<tr>';
					html += '<td width="30%" title="'+cooperationFacilitatingAgencies[i].cooperativeFacilitatingAgencyName+'">'+(cooperationFacilitatingAgencies[i].cooperativeFacilitatingAgencyName==null?'-':cooperationFacilitatingAgencies[i].cooperativeFacilitatingAgencyName)+'</td>';
					html += '<td width="40%" title="'+cooperationFacilitatingAgencies[i].serviceItems+'">'+(cooperationFacilitatingAgencies[i].serviceItems==null?'-':cooperationFacilitatingAgencies[i].serviceItems)+'</td>';
					html += '<td width="30%">'+(cooperationFacilitatingAgencies[i].signingTime==null?'-':cooperationFacilitatingAgencies[i].signingTime)+'</td>';
					html += '</tr>';
				}
		...
		success: function(data, textStatus){
			...
			if(status == "1"){
				...
				for(var i=0;i<serviceFunctions.length;i++){
					if(serviceFunctions[i].serviceItems == null &&
							serviceFunctions[i].serviceScale == null &&
							serviceFunctions[i].proportion == null){
						continue;
					}
					html += '<tr>';
					html += '<td width="30%" title="'+serviceFunctions[i].serviceItems+'">'+(serviceFunctions[i].serviceItems==null?'-':serviceFunctions[i].serviceItems)+'</td>';
					html += '<td width="40%" title="'+serviceFunctions[i].serviceScale+'">'+(serviceFunctions[i].serviceScale==null?'-':serviceFunctions[i].serviceScale)+'</td>';
					html += '<td width="30%">'+(serviceFunctions[i].proportion==null?'-':serviceFunctions[i].proportion)+'</td>';
					html += '</tr>';
				}
			}
...
function getPersonnelListByParams(){
	...
	$.ajax({
		type: "post",
		url: rootPath+"/exemplaryMatrixApplication/getPersonnelListByParamsNoPage.action",
		async : false,
		timeout : 30000,
		dataType:'json',
		data: params,
		...
		success: function(data, textStatus){
			...
			if(status == "1"){
				...
				for(var i=0;i<personnelLists.length;i++){
					...
					html += '<td width="12%" title="'+personnelLists[i].duty+'">'+personnelLists[i].duty+'</td>';
					html += '<td width="13%" title="'+personnelLists[i].professionalTitle+'">'+personnelLists[i].professionalTitle+'</td>';
					html += '<td width="20%" title="'+personnelLists[i].jobContent+'">'+personnelLists[i].jobContent+'</td>';
					html += '</tr>';
				}
		...
		success: function(data, textStatus){
			...
			if(status == "1"){
				...
				for(var i=0;i<enterpriseEvaluates.length;i++){
					...
					html += '<td width="13%" title="'+enterpriseEvaluates[i].enterpriseName+'">'+enterpriseEvaluates[i].enterpriseName+'</td>';
					...
					html += '<td width="20%" title="'+enterpriseEvaluates[i].enjoyService+'">'+enterpriseEvaluates[i].enjoyService+'</td>';
