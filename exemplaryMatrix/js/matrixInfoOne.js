...
//	}
	...
	$("#cancel").click(function(){
		window.location.href = rootPath+"/page/jsp/exemplaryMatrixNew/matrixInfoTwo.jsp";
	})
	...
	$.ajax({
		...
		success: function(data, textStatus){
			...
			if(1==status){
				...
				//注册资本
				$("#registeredCapital").val(enterpriseInfo.registeredCapital==null?"":enterpriseInfo.registeredCapital+" 万元");
				//企业网址
...
function saveCheck(){
	...
	if(flag){
		...
		var isPhone = /^0\d{2,3}-\d{7,8}(-\d{1,6})?$/;
		var isPhone2 = /^\d{7,8}(-\d{1,6})?$/;
		if($.trim(phone)!="" && reg.test(phone)==false && isPhone.test(phone)==false && isPhone2.test(phone)==false){
			alert("联系电话格式不正确！");
			$("#contactPhone_error").text("联系电话格式不正确");
			$("#contactPhone_error").show();
			flag = false;
