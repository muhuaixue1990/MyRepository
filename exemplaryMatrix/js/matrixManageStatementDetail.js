...
function getCompleteInfo(){
	...
	//清空列表
	$("#districtAuditStatementList").next("tr").remove();
	$.ajax({
		type: "post",
		url: rootPath+"/exemplaryMatrixBackstage/findStatementAuditListNoPage.action",
		async : false,
		timeout : 30000,
		dataType:'json',
		data: params,
		...
		success: function(data, textStatus){
			...
			if(statementAuditList.length > 0){
				for(var i=0; i<statementAuditList.length; i++){
					...
					if(statementAuditList[i].coopService.length>10){
						var coopService = "";
						coopService = statementAuditList[i].coopService.substring(0,10);
						html += '<td title="'+statementAuditList[i].coopService+'">'+coopService+'...</td>';
					}else{
					...
					if(statementAuditList[i].service.length>10){
						var service = ""
						service = statementAuditList[i].service.substring(0,10);
						html += '<td title="'+statementAuditList[i].service+'">'+service+'...</td>';
					}else{
				...
				html += "<tr><td colspan='15'>未查询到数据或该季度的数据未通过审核！</td></tr>"
			}
			$("#districtAuditStatementList").after(html);
		}
	})
}
...
function getServiceData(){
	var params = {'params.userCode':matrixCode,
					...
					'params.serviceQuarter':$("#quarter").val(),
					'params.auditType':'1'};
	var paramYear = $("#year").val();
	var paramQuarter = $("#quarter").val();
	$.ajax({
		type: "post",
