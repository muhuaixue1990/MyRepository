...
$(function(){
	...
	$("#matrixList").on("click",".examine",function(){
		...
		var operating = $(this).attr("id");
		
		//var applicationYear = $("#applicationYearList").val();
		
		if(operating == undefined){
			window.location.href = rootPath+"/page/jsp/exemplaryMatrix/thirdPartyExamineInfoOne.jsp?applicationUserCode="+applicationUserCode;
		}else{
			var operatingAttr = operating.split(",");
			window.location.href = rootPath+"/page/jsp/exemplaryMatrix/thirdPartyExamineInfoOne.jsp?applicationUserCode="+applicationUserCode+"&auditType="+operatingAttr[0]+"&auditFlag="+operatingAttr[1];
		}
...
function getList(params){
	...
	$.ajax({
		type: "post",
		url: rootPath+"/exemplaryMatrixApplication/thirdPartyAuditGetList.action",
		async : false,
		timeout : 30000,
		dataType:'json',
		data: params,
		...
		success: function(data, textStatus){
			...
			if(matrixList.length > 0){
				for(var i=0; i<matrixList.length; i++){
					...
					if(undefined == matrixList[i].thirdPartyAuditTime){
						html += '<td width="12%"></td>';
					}else{
						html += '<td width="12%">'+matrixList[i].thirdPartyAuditTime.substring(0,10)+'</td>';
					}
					...
					//市级未进行最终的审核，均可以进行推荐
					if(matrixList[i].cityAuditType == "0" || matrixList[i].cityAuditType == "3"){
						auditFlag = true;
					}
					...
					if(matrixList[i].thirdPartyAuditType == "0"){
						html += '<td width="10%">待推荐</td>';
						html += '<td width="12%" class="link_blue examine" name="'+matrixList[i].userCode+'" >推荐</td>';
					}else if(matrixList[i].thirdPartyAuditType == "1"){
						html += '<td width="10%">不推荐</td>';
						html += '<td width="12%" class="link_blue examine" name="'+matrixList[i].userCode+'" id="1,false">查看</td>';
					}else if(matrixList[i].thirdPartyAuditType == "2"){
						html += '<td width="10%">已推荐</td>';
						html += '<td width="12%" class="link_blue examine" name="'+matrixList[i].userCode+'" id="2,false">查看</td>';
					}else{
						html += '<td width="10%"></td>';
					}
